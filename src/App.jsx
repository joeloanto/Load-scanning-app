...
async function renderPDF(file) {
  ...
  try {
    ...
    // Try to extract Project ID from text (common patterns)
    const mProj = text.match(/[Pp]roject\s*[Ii][Dd]?\s*[:\-#]?\s*([A-Za-z0-9\-_]+)/);
    if (mProj) projectId = mProj[1];
  } catch (e) {}

  // If either orderId or projectId is missing, use AI to extract both
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: "text",
                text: `Extract the following fields from this document and reply ONLY with a JSON object, no markdown, no extra text:
{
  "orderId": "<PO number, e.g. PO0000050491, or null if not found>",
  "projectId": "<Project ID value, or null if not found>"
}`
              }
            ]
          }]
      ...
      if (block) {
        try {
          ...
          // Fallback: try regex on raw text if JSON parse fails
          if (!orderId) {
  ...
  return { orderId, projectId, date, pages, filename: file.name };
}
              ...
              {orders.map((o, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <button onClick={() => onSelect(o)}
                    ...
                    <div style={{ fontSize: 11, color: "#64748b", marginTop: 1 }}>{o.pages.length} page(s) · {o.date || o.filename}</div>
                  </button>
    ...
    <div style={{ minHeight: "100vh", background: "#222", display: "flex", flexDirection: "column", fontFamily: "'Segoe UI',sans-serif", userSelect: "none" }}>
      ...
      <div style={{ background: "#1e293b", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 2px 8px rgba(0,0,0,0.4)", flexShrink: 0 }}>
        ...
        <div style={{ textAlign: "center" }}>
          ...
          {order.projectId && (
            <div style={{ fontSize: 10, color: "#f5a623", fontWeight: 600 }}>Project: {order.projectId}</div>
          )}
